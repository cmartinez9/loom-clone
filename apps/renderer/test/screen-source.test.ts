 ...
 * `hasSourceFrameAt` and `prime` go through the same part selection and the same
 * conversion, and what they touch is observable — one is the index, the other is a
 * byte read against a named URL.
 */
 ...
 */
async function open(starts: readonly number[] = [0, 5]): Promise<Harness> {

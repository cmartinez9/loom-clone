...
and out of order because they are self-contained — and the editor window that finally
puts the decode path, the compositor and the timeline model in front of a person.
...
and the editor one way at a time — editing the production source on disk — and requiring
a test to fail each time. The one property it deliberately leaves uncovered, because no
test in `npm test` can catch it, is named in `AGENTS.md` rather than papered over.

...
keyframe, zoom, annotation and generator controls that reach them.
...
and the editor with its controls one way at a time — editing the production source on
disk rather than a copy of it — and requiring a test to fail each time.
`scripts/mutation-check.mjs`'s registry is what it breaks and where that list lives.
The one property it deliberately leaves uncovered, because no test in `npm test` can
catch it, is named in `AGENTS.md` rather than papered over.
